...
# --- CẤU HÌNH TRANG WEB ---
st.set_page_config(
    page_title='Quản Lý Học Sinh Học Thêm', layout='wide', page_icon='📚'
)
...
def check_password():
  ...
  st.markdown(
      "<h2 style='text-align: center; color: #1E3A8A;'>📚 Phần Mềm Quản Lý Dạy"
      " Thêm Tại Nhà</h2>",
...
def get_active_schedule_for_date(engine, check_date):
  ...
  query_base = f"""
        SELECT l.hoc_sinh_id, h.ho_ten, h.lop_hoc, h.mon_hoc, h.hoc_phi_buoi, l.ca_hoc, 'Lịch gốc' AS nguon
...
def sync_weekly_schedule_to_google(
    calendar_id='a.luongxdnb@gmail.com', days_ahead=7
):
  try:
  ...
  try:
      ...
      if evt.get('summary', '').startswith('🏫 Dạy Thêm Ca'):
        try:
          service.events().delete(
              calendarId=calendar_id, eventId=evt['id']
          ).execute()
        except Exception:
          pass
    ...
    for i in range(days_ahead):
      ...
      for ca, group_ca in df_day.groupby('ca_hoc'):
        start_time_str, end_time_str = ca_hoc_time.get(
            ca, ('17:30:00', '19:30:00')
        )
...
def get_schedule_matrix_df(
    engine, filter_lop=None, filter_hs_id=None, ref_date=None
):
  if ref_date is None:
    ref_date = date.today()
...
def render_schedule_matrix(engine, ref_date=None):
  ...
  if df_matrix.empty:
    st.info(
        '💡 Chưa có lịch học tuần nào được thiết lập trong hệ thống cho mốc thời'
        ' gian này.'
    )
    return
...
def create_weekly_schedule_image(
    title_target, df_matrix, ref_date=None, prefix='Học sinh / Lớp: '
):
  if ref_date is None:
    ref_date = date.today()
  ...
  fig, ax = plt.subplots(
      figsize=(
          24,
          len(df_matrix) * max(1.8, max_lines_overall * 0.65) + 5.0,
      )
  )
  ax.axis('off')
  ax.axis('tight')
  ...
  week_text = (
      f"(Tuần từ {start_w.strftime('%d/%m/%Y')} đến"
      f" {end_w.strftime('%d/%m/%Y')})"
  )
  ...
  ax.text(
      0.5,
      1.15,
      'THỜI KHÓA BIỂU LỊCH HỌC HÀNG TUẦN',
  ...
  plt.figtext(
      0.5,
      0.02,
      'Ghi chú: Lịch học được áp dụng ổn định cho các tuần tiếp theo.',
      ha='center',
      fontsize=10.5,
      style='italic',
      color='#475569',
...
def create_tuition_slip_image(
  ...
  if is_multi and details_list:
    ax.text(
        0.1,
        y_pos,
        'Chi tiết học phí theo các tháng & đơn giá:',
        fontsize=11.5,
        fontweight='bold',
        color='#1E3A8A',
        transform=ax.transAxes,
    ...
    for d in details_list:
      ...
      bd_text = (
          ' + '.join(bd_parts)
          if bd_parts
          else f"{d['so_ca']} ca x {price_per_lesson:,.0f}đ"
      )
      line_d = (
          f"• Tháng {d['thang_key']}: ({bd_text}) ="
          f" {d['thanh_tien']:,.0f}đ [{d['trang_thai']}]"
      )
      ax.text(
          0.12,
          y_pos,
...
def create_student_attendance_history_image(
    student_name, lop_hoc, month_year, df_history, total_present
):
  ...
  ax.text(
      0.5,
      0.94,
      'LỊCH SỬ ĐIỂM DANH & NHẬN XÉT HỌC SINH',
      fontsize=15,
      fontweight='bold',
      color='#1E3A8A',
      ha='center',
  ...
  ax.text(
      0.5,
      0.89,
      f'Học sinh: {student_name} - Lớp: {lop_hoc} ({month_year})',
      fontsize=12,
      fontweight='bold',
      color='#0F172A',
      ha='center',
  ...
  ax.text(
      0.5,
      0.84,
      f'Tổng số buổi đi học (Có mặt): {total_present} buổi',
      fontsize=11,
      fontweight='bold',
      color='#B91C1C',
      ha='center',
  ...
  table = ax.table(
      ...
      loc='center',
      cellLoc='center',
      colWidths=[0.15, 0.18, 0.18, 0.15, 0.34],
  )
  table.auto_set_font_size(False)
...
if st.sidebar.button(
    '🚪 Đăng xuất', type='secondary', use_container_width=True
):
  st.session_state.logged_in = False
  st.rerun()
...
user_gmail = st.sidebar.text_input(
    'Địa chỉ Gmail trên iPhone:', value='a.luongxdnb@gmail.com'
)

if st.sidebar.button(
    '🔄 Đồng Bộ Lịch 7 Ngày Tới Sang iPhone', type='primary'
):
  target_cal_id = user_gmail.strip() if user_gmail.strip() else 'primary'
  success, msg = sync_weekly_schedule_to_google(
      calendar_id=target_cal_id, days_ahead=7
  )
  if success:
...
qr_file = st.sidebar.file_uploader(
    'Tải lên ảnh Mã QR (VietQR/STK)', type=['png', 'jpg', 'jpeg']
)
...
if os.path.exists('qr_code.png'):
  st.sidebar.image(
      'qr_code.png', caption='Mã QR thanh toán hiện tại', use_container_width=True
  )

st.sidebar.markdown('---')
st.sidebar.info(
    '☁️ Dữ liệu đang được kết nối trực tiếp và lưu trữ vĩnh viễn trên **Supabase'
    ' Cloud**.'
)
...
if choice == '0. 📊 Trang Chủ Dashboard':
  ...
  query_unpaid_details = f"""
        SELECT h.id, h.ho_ten, h.lop_hoc, h.hoc_phi_buoi,
               ...
               COUNT(d.id) AS so_ca,
               SUM(COALESCE(d.don_gia, h.hoc_phi_buoi)) AS tien_no
        FROM hoc_sinh h
  ...
  with col1:
    ...
    st.metric(
        '🏫 Ca dạy hôm nay', f'{total_ca} ca', f'{total_hs_today} lượt học sinh'
    )
  with col2:
    st.metric(
        '💳 Học sinh chưa đóng phí',
        f'{unique_unpaid_students} em',
        'Trong 1 năm qua (trừ tháng này)',
    )
  with col3:
    st.metric(
        '💰 Tổng tiền còn cần thu',
        f'{total_debt_amount:,.0f} đ',
        'Các tháng trước',
    )

  st.markdown('---')
  st.markdown(
      '#### 📋 Chi Tiết Danh Sách Học Sinh Chưa Đóng Học Phí (1 Năm Qua, Trừ'
      ' Tháng Này):'
  )
  if df_unpaid_details.empty:
    st.success(
        '✅ Tuyệt vời! Tất cả học sinh trong 1 năm qua (trừ tháng này) đã hoàn'
        ' thành học phí.'
    )
  else:
    display_debt_df = df_unpaid_details[
        ['ho_ten', 'lop_hoc', 'thang_nam', 'so_ca', 'tien_no']
    ].copy()
    display_debt_df.columns = [
        'Họ và Tên',
        'Lớp',
        'Tháng Chưa Đóng',
        'Số Ca Học',
    ...
    display_debt_df['Số Tiền Cần Thu (VNĐ)'] = display_debt_df[
        'Số Tiền Cần Thu (VNĐ)'
    ].map('{:,.0f} đ'.format)
  ...
  st.markdown('---')
  st.markdown(
      '#### 🏫 Chi Tiết Lịch Dạy & Học Sinh Hôm Nay (Sắp xếp từ sớm đến muộn):'
  )
  if df_today.empty:
    st.info('💡 Hôm nay không có ca dạy nào được lên lịch.')
  else:
    sorted_cas_today = sorted(
        df_today['ca_hoc'].unique().tolist(), key=ca_hoc_sort_key
    )
    for ca in sorted_cas_today:
      group_ca = df_today[df_today['ca_hoc'] == ca]
      with st.expander(
          f'⏰ Ca: {ca} ({len(group_ca)} học sinh)', expanded=True
      ):
...
# =========================================================
  ...
  with tab_dd_moi:
    ...
    st.caption(
        f"Ngày được chọn: **{ngay_hoc.strftime('%d/%m/%Y')} ({thu_hom_nay})**"
    )

    df_active_today = get_active_schedule_for_date(engine, ngay_hoc)
    df_all_hs = pd.read_sql_query(
        'SELECT id AS hoc_sinh_id, ho_ten, lop_hoc, mon_hoc, hoc_phi_buoi FROM'
        ' hoc_sinh',
        engine,
    )
    ...
    che_do_nguon = st.radio(
        ...
        [
            ...
            (
                '2. Điểm danh học sinh / lớp KHÔNG có lịch học hôm nay (Học'
                ' bù, phát sinh...)'
            ),
        ],
    ...
    if df_all_hs.empty:
      ...
      if che_do_nguon.startswith('1.'):
        ...
        available_classes = sorted(
            df_all_hs['lop_hoc'].dropna().unique().tolist()
        )
        class_options_lbl = [f'🏫 Cả Lớp: {cls}' for cls in available_classes]
        student_dict = {
            (
                f"👤 {row['ho_ten']} [{row['lop_hoc']}] -"
                f" ID:{row['hoc_sinh_id']}"
            ): row['hoc_sinh_id']
            for _, row in df_all_hs.iterrows()
        }

        all_options_opt2 = [
            '-- Vui lòng chọn Lớp hoặc Học sinh cần điểm danh --'
        ] + class_options_lbl + list(student_dict.keys())
        selected_opt2 = st.selectbox(
            '🔍 Tìm kiếm và chọn Lớp hoặc Học sinh:',
            all_options_opt2,
            key='sel_opt2_custom',
        )

        if selected_opt2.startswith('🏫 Cả Lớp:'):
          selected_lop_name = selected_opt2.replace(
              '🏫 Cả Lớp: ', ''
          ).strip()
          df_lop_filtered = df_all_hs[
              df_all_hs['lop_hoc'] == selected_lop_name
          ].copy()
          ...
          df_hs_filtered = df_all_hs[
              df_all_hs['hoc_sinh_id'] == sel_hs_id_val
          ].copy()
      ...
      if target_students.empty:
        st.info(
            'ℹ️ Chưa có đối tượng nào được chọn hoặc không có học sinh trong'
            ' danh sách lịch học hôm nay.'
        )
      else:
        st.markdown(
            f'#### 📋 Bảng Điểm Danh ({len(target_students)} lượt học ca)'
        )
        with st.form('form_diem_danh_execution'):
          danh_sach_ca_mau_dd = DANH_SACH_CA_MAU + [
              '⏱️ Tự nhập giờ tùy chỉnh...'
          ]
          danh_sach_luu = []

          for idx, row in target_students.iterrows():
            default_hp = float(row.get('hoc_phi_buoi', 150000))
            st.markdown(
                f"**👤 {row['ho_ten']}** [{row.get('lop_hoc', 'N/A')}] - *Ca:"
                f" {row.get('ca_hoc', '17h30 - 19h30')}*"
            )
            c1, c2, c3, c4 = st.columns([2, 1.8, 2.2, 4])

            default_ca = (
                row['ca_hoc']
                if (
                    'ca_hoc' in row
                    and pd.notna(row['ca_hoc'])
                    and row['ca_hoc'] in DANH_SACH_CA_MAU
                )
                else '17h30 - 19h30'
            )
            ...
            with c1:
              ca_val = st.selectbox(
                  ...
                  danh_sach_ca_mau_dd,
                  index=(
                      danh_sach_ca_mau_dd.index(default_ca)
                      if default_ca in danh_sach_ca_mau_dd
                      else 5
                  ),
                  key=f"ca_cls_{row['hoc_sinh_id']}_{idx}",
              )
              if ca_val == '⏱️ Tự nhập giờ tùy chỉnh...':
                custom_ca = st.text_input(
                    'Nhập giờ',
                    value='18h00 - 20h00',
                    key=f"custom_ca_{row['hoc_sinh_id']}_{idx}",
                )
            ...
            with c2:
              stt_val = st.radio(
                  ...
                  index=0,
                  key=f"stt_cls_{row['hoc_sinh_id']}_{idx}",
                  horizontal=False,
              )

            with c3:
              don_gia_val = st.number_input(
                  'Đơn giá ca (đ)',
                  min_value=0,
                  step=10000,
                  value=int(default_hp),
                  key=f"dg_cls_{row['hoc_sinh_id']}_{idx}",
              )
            ...
            with c4:
              ...
              selected_tags = st.multiselect(
                  '🏷️ Thẻ thái độ:',
                  tags_options,
                  key=f"tags_cls_{row['hoc_sinh_id']}_{idx}",
              )
              custom_nx = st.text_input(
                  'Ghi chú thêm',
                  key=f"nx_cls_{row['hoc_sinh_id']}_{idx}",
          ...
          if st.form_submit_button(
              '💾 LƯU ĐIỂM DANH', type='primary', use_container_width=True
          ):
            success_count = 0
            with engine.begin() as conn:
                ...
                try:
                  existing_record = conn.execute(
                      text("""
                                ...
                                """),
                      {'hs_id': item[0], 'ngay': item[1], 'ca': item[2]},
                  ).fetchone()
            ...
            if success_count > 0:
              st.success(
                  f'✅ Đã lưu thành công {success_count} bản ghi điểm danh lên'
                  ' Supabase!'
              )
              st.rerun()
    ...
    st.markdown('---')
    st.subheader(
        f"📊 Kết quả & Thống kê điểm danh ngày {ngay_hoc.strftime('%d/%m/%Y')}"
    )

    df_dd_today = pd.read_sql_query(
        f"""
            SELECT d.id, h.ho_ten AS "Họ và Tên", h.lop_hoc AS "Lớp", d.ca_hoc AS "Ca Học", d.trang_thai AS "Trạng Thái", d.nhan_xet AS "Nhận Xét", COALESCE(d.don_gia, h.hoc_phi_buoi) AS "Đơn Giá"
            FROM diem_danh d
    ...
    if not df_dd_today.empty:
      ...
      vang_khong_phep = len(
          df_dd_today[df_dd_today['Trạng Thái'] == 'Vắng không phép']
      )
      ...
      df_dd_today['Đơn Giá'] = df_dd_today['Đơn Giá'].map('{:,.0f} đ'.format)
      st.dataframe(
          df_dd_today[
              ['Họ và Tên', 'Lớp', 'Ca Học', 'Trạng Thái', 'Đơn Giá', 'Nhận Xét']
          ],
  ...
  with tab_dd_quanly:
    ...
    sel_date_filter = st.date_input(
        '🗓️ Chọn ngày cần sửa/xóa điểm danh',
        date.today(),
        key='filter_log_date_picker',
    )
    date_filter_str = sel_date_filter.strftime('%Y-%m-%d')

    df_logs = pd.read_sql_query(
        f"""
            SELECT d.id AS "Mã Lịch", h.id AS hoc_sinh_id, h.ho_ten AS "Họ Tên", h.lop_hoc AS "Lớp", d.ngay AS "Ngày", d.ca_hoc AS "Ca Học", d.trang_thai AS "Trạng Thái", d.nhan_xet AS "Nhận Xét", COALESCE(d.don_gia, h.hoc_phi_buoi) AS "Đơn Giá" 
            FROM diem_danh d 
            JOIN hoc_sinh h ON d.hoc_sinh_id = h.id 
    ...
    if not df_logs.empty:
      st.write(
          f"📋 Danh sách điểm danh trong ngày"
          f" **{sel_date_filter.strftime('%d/%m/%Y')}** ({len(df_logs)} bản ghi):"
      )
      df_logs_show = df_logs.copy()
      df_logs_show['Đơn Giá'] = df_logs_show['Đơn Giá'].map(
          '{:,.0f} đ'.format
      )
      st.dataframe(
          df_logs_show[
      ...
      with st.expander('⚙️ Sửa / Xóa từng bản ghi điểm danh lẻ'):
        log_dict = {
            (
                f"Mã ID: {row['Mã Lịch']} - {row['Họ Tên']} [{row['Lớp']}] (Ca:"
                f" {row['Ca Học']} - {row['Trạng Thái']})"
            ): row['Mã Lịch']
            for _, row in df_logs.iterrows()
        }
        selected_log_label = st.selectbox(
            'Chọn bản ghi (Mã Lịch) cần sửa hoặc xóa:',
            list(log_dict.keys()),
            key='log_sel_id_by_date',
        )
        ...
        with st.form('form_edit_delete_diem_danh_record'):
          st.write(
              f"Đang thao tác Mã Lịch **{log_to_edit_del}**:"
              f" {row_log_item['Họ Tên']} [{row_log_item['Lớp']}] - Ngày:"
              f" {row_log_item['Ngày']} - Ca: {row_log_item['Ca Học']}"
          )
          stt_options = ['Có mặt', 'Vắng có phép', 'Vắng không phép']
          default_stt_idx = (
              stt_options.index(row_log_item['Trạng Thái'])
              if row_log_item['Trạng Thái'] in stt_options
              else 0
          )
          edit_stt_val = st.selectbox(
              'Trạng thái mới:',
              stt_options,
              index=default_stt_idx,
              key='edit_log_stt',
          )
          edit_dg_val = st.number_input(
              'Đơn giá ca (đ):',
              min_value=0,
              step=10000,
              value=int(row_log_item['Đơn Giá']),
              key='edit_log_dg',
          )
          edit_nx_val = st.text_input(
              'Nhận xét mới:',
              value=row_log_item['Nhận Xét'] or '',
              key='edit_log_nx',
          )

          col_eb1, col_eb2 = st.columns(2)
          with col_eb1:
            submit_update_log = st.form_submit_button(
                '💾 Cập Nhật Điểm Danh', type='primary'
            )
          with col_eb2:
            submit_delete_log = st.form_submit_button(
                '❌ Xóa Bản Ghi Này', type='secondary'
            )
      ...
      st.info(
          f"💡 Không có bản ghi điểm danh nào trong ngày"
          f" {sel_date_filter.strftime('%d/%m/%Y')}."
      )

  with tab_dd_lich_su:
    st.subheader(
        '📊 Xem Lịch Sử Điểm Danh & Xuất Ảnh Theo Học Sinh Trong Tháng'
    )
    df_hs_ls = pd.read_sql_query(
        'SELECT id, ho_ten, lop_hoc FROM hoc_sinh ORDER BY id DESC', engine
    )
    if df_hs_ls.empty:
      ...
      with c_hs_ls:
        hs_dict_ls = {
            f"{r['ho_ten']} [{r['lop_hoc']}] - ID:{r['id']}": r['id']
            for _, r in df_hs_ls.iterrows()
        }
        sel_hs_ls_lbl = st.selectbox(
            'Chọn học sinh', list(hs_dict_ls.keys()), key='sel_hs_ls_key'
        )
      ...
      df_hs_att_history = pd.read_sql_query(
          f"""
                SELECT 
                    TO_CHAR(d.ngay, 'DD/MM/YYYY') AS "Ngày",
                    d.ca_hoc AS "Ca học",
                    d.trang_thai AS "Trạng thái",
                    COALESCE(d.don_gia, h.hoc_phi_buoi) AS "Đơn giá",
      ...
      if df_hs_att_history.empty:
        st.info(
            f"ℹ️ Học sinh {sel_hs_row_ls['ho_ten']} chưa có lịch sử điểm danh"
            f' trong Tháng {thang_nam_k}.'
        )
      else:
        total_co_mat = len(
            df_hs_att_history[df_hs_att_history['Trạng thái'] == 'Có mặt']
        )
        st.metric(
            '🟢 Tổng số buổi đi học (Có mặt)',
            f'{total_co_mat} buổi',
            f"Tổng số bản ghi: {len(df_hs_att_history)} buổi",
        )
        df_show_hist = df_hs_att_history.copy()
        df_show_hist['Đơn giá'] = df_show_hist['Đơn giá'].map(
            '{:,.0f} đ'.format
        )
        st.dataframe(df_show_hist, use_container_width=True)
        ...
        if HAS_MATPLOTLIB:
          img_ls_bytes = create_student_attendance_history_image(
              ...
              df_history=df_show_hist,
              total_present=total_co_mat,
          )
          safe_name_hs = re.sub(
              r'[\\/*?:"<>|]',
              '',
              f"{sel_hs_row_ls['ho_ten']}_{sel_hs_row_ls['lop_hoc']}".replace(
                  ' ', '_'
              ),
          )
          st.download_button(
              label=(
                  f"🖼️ Tải Ảnh Lịch Sử Điểm Danh ({sel_hs_row_ls['ho_ten']})"
              ),
              data=img_ls_bytes,
              file_name=(
                  f'Lich_Su_Diem_Danh_{safe_name_hs}_Thang_{thang_ls}_{nam_ls}.png'
              ),
              mime='image/png',
              type='primary',
...
# =========================================================
  ...
  tab_matrix, tab_goc, tab_export = st.tabs(
      ['🗺️ Lịch Học Tổng Quan', '📅 Lịch Gốc Hàng Tuần', '📥 Xuất Ảnh Lịch Học']
  )

  with tab_matrix:
    st.markdown('##### 🗓️ Chọn mốc tuần cần xem lịch học tổng quan:')
    sel_date_matrix = st.date_input(
        'Xem tuần chứa ngày:', date.today(), key='sel_date_matrix_main'
    )
    st.divider()
  ...
  with tab_goc:
    ...
    sub_tab_goc_tao, sub_tab_goc_sua = st.tabs(
        ['➕ Thêm lịch gốc mới', '✏️ Sửa lịch học gốc']
    )

    df_hs = pd.read_sql_query(
        'SELECT id, ho_ten, lop_hoc, mon_hoc FROM hoc_sinh', engine
    )
    ...
    with sub_tab_goc_tao:
      if df_hs.empty:
        ...
        mode_goc = st.radio(
            'Phạm vi xếp lịch gốc:',
            [
                'Theo Lớp (Áp dụng chung cả lớp)',
                'Theo Từng Học Sinh Riêng Biệt',
            ],
            horizontal=True,
            key='mode_goc_sched',
        )
        ...
        if mode_goc == 'Theo Lớp (Áp dụng chung cả lớp)':
          ...
          selected_lop = st.selectbox(
              'Chọn Lớp để xếp lịch gốc', all_lops, key='select_goc_lop'
          )
          target_hs_ids = df_hs[df_hs['lop_hoc'] == selected_lop][
              'id'
          ].tolist()
          target_name_label = f'Lớp {selected_lop}'
        else:
          hs_dict_goc = {
              f"{row['ho_ten']} [{row['lop_hoc']}] - ID:{row['id']}": row['id']
              for _, row in df_hs.iterrows()
          }
          selected_hs_label = st.selectbox(
              'Chọn Học sinh cụ thể:',
              list(hs_dict_goc.keys()),
              key='select_goc_hs_indiv',
          )
        ...
        for t in cac_thu:
          with st.expander(f'🗓️ {t}', expanded=True):
            ...
            custom_ca_them = st.text_input(
                f'Thêm ca giờ tùy chỉnh vào {t} (nếu có, cách nhau bằng dấu'
                ' phẩy):',
                placeholder='VD: 08h00 - 10h00',
            ...
            if custom_ca_them.strip():
              extra_cas = [
                  c_item.strip()
                  for c_item in custom_ca_them.split(',')
                  if c_item.strip()
              ]
        ...
        if st.button(
            f'💾 Lưu Lịch Học Gốc Cho {target_name_label}', type='primary'
        ):
          with engine.begin() as conn:
            for hs_id in target_hs_ids:
              conn.execute(
                  text(
                      'DELETE FROM lich_hoc_tuan WHERE hoc_sinh_id = :id'
                  ),
                  {'id': hs_id},
              )
          ...
          st.success(
              f'✅ Đã lưu lịch gốc cho {target_name_label} thành công lên'
              ' Supabase!'
          )
          st.rerun()
    ...
    with sub_tab_goc_sua:
      ...
      if df_goc_all.empty:
        st.info(
            '💡 Chưa có lịch học gốc nào được thiết lập trong hệ thống.'
        )
      else:
        xoa_mode = st.radio(
            'Chọn phạm vi xóa lịch gốc:',
            ['Xóa theo Lớp', 'Xóa theo Học sinh cụ thể'],
            horizontal=True,
            key='xoa_mode_goc_scope',
        )

        if xoa_mode == 'Xóa theo Lớp':
          all_lops_goc = sorted(df_goc_all['lop_hoc'].dropna().unique().tolist())
          selected_lop_xoa = st.selectbox(
              'Chọn Lớp cần xóa lịch gốc:', all_lops_goc, key='sel_lop_xoa_goc'
          )
          df_lop_goc_records = df_goc_all[
              df_goc_all['lop_hoc'] == selected_lop_xoa
          ]

          st.write(
              f"📋 Danh sách các ca học gốc của lớp"
              f" **{selected_lop_xoa}** ({len(df_lop_goc_records)} bản ghi):"
          )
          if not df_lop_goc_records.empty:
            st.dataframe(
                df_lop_goc_records[['id', 'ho_ten', 'thu', 'ca_hoc']],
                use_container_width=True,
            )

            with st.form(f'form_xoa_goc_lop_{selected_lop_xoa}'):
              unique_slots = (
                  df_lop_goc_records[['thu', 'ca_hoc']]
                  .drop_duplicates()
                  .values.tolist()
              )
              ...
              confirm_del_lop = st.checkbox(
                  f'Tôi xác nhận muốn xóa các ca học đã chọn của lớp'
                  f' {selected_lop_xoa}'
              )
              sub_btn_del_lop = st.form_submit_button(
                  '❌ Xóa Lịch Học Gốc Đã Chọn', type='primary'
              )
              ...
              if sub_btn_del_lop:
                if confirm_del_lop:
                  if not sel_slots_to_delete:
                    st.warning(
                        '⚠️ Vui lòng chọn ít nhất một ngày và ca học để xóa!'
                    )
                  else:
                    hs_ids_in_lop = (
                        df_lop_goc_records['hoc_sinh_id'].unique().tolist()
                    )
                    with engine.begin() as conn:
                    ...
                    st.success(
                        f'✅ Đã xóa các ngày và ca học đã chọn của lớp'
                        f' {selected_lop_xoa}!'
                    )
                    st.rerun()
                else:
                  st.warning(
                      '⚠️ Vui lòng tích chọn xác nhận để thực hiện xóa.'
                  )
          else:
          ...
          if df_hs_goc.empty:
            ...
            hs_dict_xoa_goc = {
                (
                    f"{row['ho_ten']} [{row['lop_hoc']}] - ID:{row['id']}"
                ): row['id']
                for _, row in df_hs_goc.iterrows()
            }
            sel_hs_xoa_goc_lbl = st.selectbox(
                'Chọn học sinh cụ thể cần xóa lịch học gốc:',
                list(hs_dict_xoa_goc.keys()),
                key='sel_hs_xoa_goc_key',
            )
            sel_hs_id_goc = hs_dict_xoa_goc[sel_hs_xoa_goc_lbl]

            df_single_goc = df_goc_all[
                df_goc_all['hoc_sinh_id'] == sel_hs_id_goc
            ]
            st.write('📋 Lịch gốc hiện tại của học sinh:')
            st.dataframe(
                df_single_goc[['id', 'thu', 'ca_hoc']], use_container_width=True
            )

            with st.form(f'form_xoa_goc_hs_{sel_hs_id_goc}'):
              unique_slots_hs = (
                  df_single_goc[['thu', 'ca_hoc']]
                  .drop_duplicates()
                  .values.tolist()
              )
              ...
              confirm_del_hs = st.checkbox(
                  'Tôi xác nhận muốn xóa các ca học đã chọn của học sinh này'
              )
              sub_btn_del_hs = st.form_submit_button(
                  '❌ Xóa Lịch Học Gốc Đã Chọn', type='primary'
              )
              ...
              if sub_btn_del_hs:
                if confirm_del_hs:
                  if not sel_slots_hs_to_delete:
                    ...
                    st.success(
                        '✅ Đã xóa các ngày và ca học đã chọn của học sinh!'
                    )
                    st.rerun()
                else:
                  st.warning(
  ...
  with tab_export:
    ...
    df_hs_all = pd.read_sql_query(
        'SELECT id, ho_ten, lop_hoc FROM hoc_sinh', engine
    )

    if df_hs_all.empty:
      st.warning('Chưa có dữ liệu học sinh.')
    else:
      sel_date_export = st.date_input(
          '🗓️ Chọn tuần để xuất ảnh:', date.today(), key='sel_date_export_img_m'
      )
      filter_mode = st.radio(
          'Chọn phạm vi xuất lịch học:',
          ['Theo Lớp cụ thể', 'Theo Học sinh cụ thể'],
          horizontal=True,
          key='filter_mode_exp_m',
      )
      ...
      if filter_mode == 'Theo Lớp cụ thể':
        ...
        selected_lop_exp = st.selectbox(
            'Chọn Lớp:', lop_list, key='sel_lop_exp_m'
        )
        target_title = f'Lớp {selected_lop_exp}'
        prefix_label = 'Học sinh / Lớp: '
        safe_lop_name = re.sub(
            r'[\\/*?:"<>|]', '', f'{selected_lop_exp}'.replace(' ', '_')
        )
        ...
        hs_dict_exp = {
            ...
            for _, row in df_hs_all.iterrows()
        }
        sel_hs_label = st.selectbox(
            'Chọn Học sinh:', list(hs_dict_exp.keys()), key='sel_hs_label_exp_m'
        )
        selected_hs_row = hs_dict_exp[sel_hs_label]
        selected_hs_exp = selected_hs_row['id']
        target_title = (
            f"{selected_hs_row['ho_ten']} ({selected_hs_row['lop_hoc']})"
        )
        prefix_label = 'Học sinh / Lớp: '
        safe_hs_name = re.sub(
            r'[\\/*?:"<>|]',
            '',
            f"{selected_hs_row['ho_ten']}_{selected_hs_row['lop_hoc']}".replace(
                ' ', '_'
            ),
        )
        file_name_download = f'Lich_Hoc_{safe_hs_name}.png'

      df_export_matrix = get_schedule_matrix_df(
          engine,
          filter_lop=selected_lop_exp,
          filter_hs_id=selected_hs_exp,
          ref_date=sel_date_export,
      )
      ...
      if df_export_matrix.empty:
        ...
        if HAS_MATPLOTLIB:
          ...
          with col_ex1:
            img_bytes = create_weekly_schedule_image(
                target_title,
                df_export_matrix,
                ref_date=sel_date_export,
                prefix=prefix_label,
          ...
          with col_ex2:
            ...
            zip_choice = st.radio(
                'Chọn nội dung file ZIP:',
                ['Tất cả học sinh', 'Tất cả các lớp'],
                horizontal=True,
                key='zip_choice_schedule',
            )

            if zip_choice == 'Tất cả học sinh':
              if st.button(
                  '📦 Tải ZIP Lịch Học TẤT CẢ Học Sinh', type='secondary'
              ):
                zip_buffer_s = io.BytesIO()
                with zipfile.ZipFile(
                    zip_buffer_s, 'w', zipfile.ZIP_DEFLATED
                ) as zf:
                  for _, hs_r in df_hs_all.iterrows():
                    ...
                    df_hs_mat = get_schedule_matrix_df(
                        engine, filter_hs_id=hs_id_val, ref_date=sel_date_export
                    )
                    if not df_hs_mat.empty:
                      img_hs_b = create_weekly_schedule_image(
                          f'{hs_name_val} ({hs_lop_val})',
                          df_hs_mat,
                          ref_date=sel_date_export,
                          prefix='Học sinh: ',
                      )
                      safe_n = re.sub(
                          r'[\\/*?:"<>|]',
                          '',
                          f'{hs_name_val}_{hs_lop_val}'.replace(' ', '_'),
                      )
                      zf.writestr(
                          f'Lich_Hoc_{safe_n}.png', img_hs_b.getvalue()
                      )
                zip_buffer_s.seek(0)
                st.download_button(
                    ...
                    data=zip_buffer_s,
                    file_name=(
                        'Tat_Ca_Lich_Hoc_Hoc_Sinh_'
                        f"{sel_date_export.strftime('%Y%m%d')}.zip"
                    ),
              ...
              if st.button(
                  '📦 Tải ZIP Lịch Học TẤT CẢ Các Lớp', type='secondary'
              ):
                zip_buffer_l = io.BytesIO()
                all_lops_list = sorted(
                    df_hs_all['lop_hoc'].dropna().unique().tolist()
                )
                with zipfile.ZipFile(
                    zip_buffer_l, 'w', zipfile.ZIP_DEFLATED
                ) as zf_l:
                  for lop_val in all_lops_list:
                    df_lop_mat = get_schedule_matrix_df(
                        engine, filter_lop=lop_val, ref_date=sel_date_export
                    )
                    if not df_lop_mat.empty:
                      img_lop_b = create_weekly_schedule_image(
                          f'Lớp {lop_val}',
                          df_lop_mat,
                          ref_date=sel_date_export,
                          prefix='Lớp: ',
                      ...
                      zf_l.writestr(
                          f'Lich_Hoc_Lop_{safe_lop_n}.png', img_lop_b.getvalue()
                      )
                zip_buffer_l.seek(0)
                st.download_button(
...
# =========================================================
  ...
  che_do_xem = st.radio(
      '⏱️ Chọn chế độ xem thống kê:',
      ['Theo Tháng', 'Theo Tuần', 'Theo Ngày'],
      horizontal=True,
  )
  ...
  if che_do_xem == 'Theo Tháng':
    ...
    with col_y:
      nam_selected = st.number_input(
          'Chọn Năm', min_value=2020, max_value=2035, value=datetime.now().year
      )
    with col_m:
    ...
    if selected_thangs:
      df_hs_all = pd.read_sql_query(
          """
                SELECT id AS hoc_sinh_id, ho_ten AS "Họ và Tên", lop_hoc AS "Lớp", mon_hoc AS "Môn Học", hoc_phi_buoi AS "Đơn Giá/Ca (VNĐ)" 
                FROM hoc_sinh
            """,
          engine,
      )
      ...
      if len(selected_thangs) == 1:
        ...
        for _, hs in df_hs_all.iterrows():
          ...
          q_bd = f"""
                        SELECT COALESCE(d.don_gia, {hs['Đơn Giá/Ca (VNĐ)']}) AS don_gia, COUNT(*) AS so_ca
                        FROM diem_danh d
                        WHERE d.hoc_sinh_id = {hs_id} AND TO_CHAR(d.ngay, 'YYYY-MM') = '{thang_nam_query}' AND d.trang_thai = 'Có mặt'
                        GROUP BY COALESCE(d.don_gia, {hs['Đơn Giá/Ca (VNĐ)']})
                    """
          df_bd = pd.read_sql_query(q_bd, engine)
          if df_bd.empty:
            continue

          so_ca_total = int(df_bd['so_ca'].sum())
          tong_tien = (df_bd['so_ca'] * df_bd['don_gia']).sum()
          breakdown_items = [
              {'so_ca': int(r['so_ca']), 'don_gia': float(r['don_gia'])}
          ...
          trang_thai_th = (
              df_pay.iloc[0]['trang_thai'] if not df_pay.empty else 'Chưa đóng'
          )
          ...
          combined_rows.append({
              ...
              'Thời gian': thang_nam_key,
              'Số Ca Có Mặt': so_ca_total,
              'Tổng Tiền (VNĐ)': tong_tien,
              'Trạng Thái': trang_thai_th,
              'is_multi': True,
              'details': [{
        ...
        for _, hs in df_hs_all.iterrows():
          ...
          for th in sorted(selected_thangs):
            ...
            so_ca_m = int(df_bd['so_ca'].sum())
            thanh_tien_m = (df_bd['so_ca'] * df_bd['don_gia']).sum()
            breakdown_items = [
                {'so_ca': int(r['so_ca']), 'don_gia': float(r['don_gia'])}
                for _, r in df_bd.iterrows()
            ]
            ...
            df_pay = pd.read_sql_query(q_pay, engine)
            trang_thai_th = (
                df_pay.iloc[0]['trang_thai']
                if not df_pay.empty
                else 'Chưa đóng'
            )

            total_ca_agg += so_ca_m
            total_tien_agg += thanh_tien_m

            valid_month_details.append({
                'thang_key': thang_nam_key,
                'so_ca': so_ca_m,
                'don_gia': hs['Đơn Giá/Ca (VNĐ)'],
                'thanh_tien': thanh_tien_m,
                'trang_thai': trang_thai_th,
                'breakdown_items': breakdown_items,
            })
          ...
          if len(valid_month_details) > 0:
            status_str_parts = [
                f"Tháng {d['thang_key']}: {d['trang_thai']}"
                for d in valid_month_details
            ]
            status_combined = ', '.join(status_str_parts)
            thangs_str = ' - '.join(
                [d['thang_key'] for d in valid_month_details]
            )
    ...
    for _, hs in df_hs_all.iterrows():
      ...
      q_bd = f"""
                SELECT COALESCE(d.don_gia, {hs['Đơn Giá/Ca (VNĐ)']}) AS don_gia, COUNT(*) AS so_ca
                FROM diem_danh d
                WHERE d.hoc_sinh_id = {hs_id} AND d.ngay = '{ngay_str}' AND d.trang_thai = 'Có mặt'
                GROUP BY COALESCE(d.don_gia, {hs['Đơn Giá/Ca (VNĐ)']})
            """
      df_bd = pd.read_sql_query(q_bd, engine)
      if df_bd.empty:
        continue
      so_ca_t = int(df_bd['so_ca'].sum())
      tong_t = (df_bd['so_ca'] * df_bd['don_gia']).sum()
      breakdown_items = [
          {'so_ca': int(r['so_ca']), 'don_gia': float(r['don_gia'])}
          for _, r in df_bd.iterrows()
      ]
      ...
      combined_rows.append({
          ...
          'Môn Học': hs['Môn Học'],
          'Đơn Giá/Ca (VNĐ)': hs['Đơn Giá/Ca (VNĐ)'],
          'Thời gian': ngay_chon.strftime('%d/%m/%Y'),
          'Số Ca Có Mặt': so_ca_t,
          'Tổng Tiền (VNĐ)': tong_t,
          'Trạng Thái': trang_thai_th,
          'is_multi': True,
          'details': [{
              'thang_key': thang_nam_key,
    ...
    st.info(
        f"📅 Thống kê tuần từ **{start_w.strftime('%d/%m/%Y')}** đến"
        f" **{end_w.strftime('%d/%m/%Y')}**"
    )
    ...
    for _, hs in df_hs_all.iterrows():
      ...
      q_bd = f"""
                SELECT COALESCE(d.don_gia, {hs['Đơn Giá/Ca (VNĐ)']}) AS don_gia, COUNT(*) AS so_ca
                FROM diem_danh d
                WHERE d.hoc_sinh_id = {hs_id} AND d.ngay >= '{start_str}' AND d.ngay <= '{end_str}' AND d.trang_thai = 'Có mặt'
                GROUP BY COALESCE(d.don_gia, {hs['Đơn Giá/Ca (VNĐ)']})
            """
      df_bd = pd.read_sql_query(q_bd, engine)
      if df_bd.empty:
      ...
      combined_rows.append({
          ...
          'Môn Học': hs['Môn Học'],
          'Đơn Giá/Ca (VNĐ)': hs['Đơn Giá/Ca (VNĐ)'],
          'Thời gian': f"Tuần {start_w.strftime('%d/%m')} - {end_w.strftime('%d/%m')}",
          'Số Ca Có Mặt': so_ca_t,
          'Tổng Tiền (VNĐ)': tong_t,
          'Trạng Thái': trang_thai_th,
          'is_multi': True,
  ...
  if not combined_df.empty and search_query.strip():
    combined_df = combined_df[
        combined_df['Họ và Tên'].str.contains(
  ...
  if combined_df.empty:
    ...
    if HAS_MATPLOTLIB:
      if st.button('📦 Xuất ZIP Hàng Loạt Phiếu Thống Kê / Hóa Đơn', type='primary'):
        ...
        with zipfile.ZipFile(
            zip_buffer_f, 'w', zipfile.ZIP_DEFLATED
        ) as zf_fee:
          for _, row_fee in combined_df.iterrows():
            ...
            safe_n_fee = re.sub(
                ...
                '',
                (
                    f"{row_fee['Họ và Tên']}_{row_fee['Lớp']}_{safe_filename_time}"
                ).replace(' ', '_'),
            )
    ...
    for idx, row in combined_df.iterrows():
      c1, c2, c3, c4, c5, c6, c7 = st.columns(
          [2.2, 1.2, 1.2, 1.2, 1.5, 1.8, 1.8]
      )
      c1.write(
          f"**{row['Họ và Tên']}**\n\n*Lớp: {row['Lớp']} ({row['Thời gian']})*"
      )
      c2.write(f"{row['Số Ca Có Mặt']} ca")
      c3.write(f"{row['Đơn Giá/Ca (VNĐ)']:,.0f} đ (Mặc định)")
      ...
      if is_multi:
        all_all_paid = all(d['trang_thai'] == 'Đã đóng' for d in details_list)
        c5.write(
            '🟢 Đã đóng tất cả'
            if all_all_paid
            else (
                '🟡 Đóng một phần'
                if any(d['trang_thai'] == 'Đã đóng' for d in details_list)
                else '🔴 Chưa đóng'
            )
        )
        btn_lbl = (
            'Xác nhận Đã đóng tất cả'
            if not all_all_paid
            else 'Chuyển Chưa đóng tất cả'
        )
      else:
      ...
      if c6.button(btn_lbl, key=f"btn_pay_{row['hoc_sinh_id']}_{idx}"):
        if is_multi:
          new_stt = (
              'Chưa đóng'
              if all(d['trang_thai'] == 'Đã đóng' for d in details_list)
              else 'Đã đóng'
          )
          ...
          new_stt = (
              'Chưa đóng' if (row['Trạng Thái'] == 'Đã đóng') else 'Đã đóng'
          )
          t_str = date.today().strftime('%Y-%m-%d') if new_stt == 'Đã đóng' else ''
          thang_nam_key_save = (
              row['Thời gian']
              if '/' in str(row['Thời gian']) and len(str(row['Thời gian'])) <= 7
...
# =========================================================
  ...
  sub_tab_them, sub_tab_sua, sub_tab_xoa = st.tabs(
      ['➕ Thêm Học Sinh', '✏️ Sửa Thông Tin', '❌ Xóa Học Sinh']
  )
  ...
  with sub_tab_them:
    with st.form('form_add_student_full'):
      ...
      with c2:
        hoc_phi_new = st.number_input(
            'Học phí mỗi ca mặc định (VNĐ)',
            min_value=0,
            step=10000,
            value=150000,
        )
          ...
          with engine.begin() as conn:
            conn.execute(
                    ...
                    """),
                {
                    ...
                    'mon': mon_new.strip(),
                    'hp': hoc_phi_new,
                    'ttph': thong_tin_phu_huynh_new.strip(),
                },
            )
  ...
  with sub_tab_sua:
    df_hs_edit = pd.read_sql_query(
        'SELECT * FROM hoc_sinh ORDER BY id DESC', engine
    )
    if not df_hs_edit.empty:
      hs_edit_dict = {
          f"{row['ho_ten']} [{row['lop_hoc']}] - ID:{row['id']}": row
          for _, row in df_hs_edit.iterrows()
      }
      selected_edit_label = st.selectbox(
          'Chọn học sinh cần sửa:', list(hs_edit_dict.keys()), key='select_edit_hs'
      )
      ...
      with st.form('form_edit_student'):
        ...
        with c1:
          ...
          lop_edit = st.text_input(
              'Lớp', value=selected_hs_row['lop_hoc'] or ''
          )
          mon_edit = st.text_input(
              'Môn', value=selected_hs_row['mon_hoc'] or ''
          )
        with c2:
          hoc_phi_edit = st.number_input(
              'Học phí mỗi ca mặc định',
              min_value=0,
              step=10000,
              value=int(selected_hs_row['hoc_phi_buoi'] or 150000),
          )
          thong_tin_phu_huynh_edit = st.text_input(
              'Thông tin phụ huynh',
              value=selected_hs_row['thong_tin_phu_huynh'] or '',
          )
          ...
          with engine.begin() as conn:
            conn.execute(
                        ...
                        """),
                {
                    ...
                    'mon': mon_edit.strip(),
                    'hp': hoc_phi_edit,
                    'ttph': thong_tin_phu_huynh_edit.strip(),
                    'id': int(selected_hs_row['id']),
                },
            )
  ...
  with sub_tab_xoa:
    df_hs_del = pd.read_sql_query(
        'SELECT id, ho_ten, lop_hoc FROM hoc_sinh ORDER BY id DESC', engine
    )
    if not df_hs_del.empty:
      hs_del_dict = {
          f"{row['ho_ten']} [{row['lop_hoc']}] - ID:{row['id']}": row['id']
          for _, row in df_hs_del.iterrows()
      }
      selected_del_id = hs_del_dict[
          st.selectbox(
              'Chọn học sinh cần xóa:',
              list(hs_del_dict.keys()),
              key='select_del_hs',
          )
      ]
      ...
      if (
          st.button('❌ XÓA HỌC SINH NÀY', type='primary')
          and confirm_check
      ):
        with engine.begin() as conn:
          conn.execute(
              text('DELETE FROM diem_danh WHERE hoc_sinh_id = :id'),
              {'id': selected_del_id},
          )
          conn.execute(
              text('DELETE FROM thanh_toan WHERE hoc_sinh_id = :id'),
              {'id': selected_del_id},
          )
          conn.execute(
              text('DELETE FROM lich_hoc_tuan WHERE hoc_sinh_id = :id'),
              {'id': selected_del_id},
          )
          conn.execute(
              text('DELETE FROM hoc_sinh WHERE id = :id'),
              {'id': selected_del_id},
          )
  ...
  st.dataframe(
      pd.read_sql_query(
          """
            SELECT id AS "Mã HS", ho_ten AS "Họ và tên", lop_hoc AS "Lớp", mon_hoc AS "Môn", hoc_phi_buoi AS "Học phí/Ca (VNĐ)", thong_tin_phu_huynh AS "Thông tin phụ huynh" 
            FROM hoc_sinh 
            ORDER BY id DESC
        """,
          engine,
