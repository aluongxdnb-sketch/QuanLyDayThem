...
# --- KẾT NỐI CƠ SỞ DỮ LIỆU ĐÁM MÂY (SUPABASE / POSTGRES) HOẶC SQLITE LOCAL ---
@st.cache_resource
def get_db_engine():
...
def check_schedule_conflicts(thu, ca_hoc, exclude_lop=None, exclude_hs_id=None):
    is_postgres = "postgres" in st.secrets and "url" in st.secrets["postgres"]
    group_func = "string_agg(h.ho_ten, ', ')" if is_postgres else "GROUP_CONCAT(h.ho_ten, ', ')"
    
    query_str = f'''
        SELECT DISTINCT h.lop_hoc, {group_func} as ds_hs
        FROM lich_hoc_tuan l
        JOIN hoc_sinh h ON l.hoc_sinh_id = h.id
        WHERE l.thu = :thu AND l.ca_hoc = :ca_hoc
    '''
    params = {"thu": thu, "ca_hoc": ca_hoc}
    if exclude_lop:
        query_str += " AND h.lop_hoc != :ex_lop"
        params["ex_lop"] = exclude_lop
    if exclude_hs_id:
        query_str += " AND h.id != :ex_hs"
        params["ex_hs"] = exclude_hs_id
    query_str += " GROUP BY h.lop_hoc"
    return pd.read_sql_query(text(query_str), engine, params=params)
...
def create_tuition_pdf(student_name, lop_hoc, subject, price_per_lesson, month_year, total_lessons, total_fee, status, qr_path):
    ...
    font_path, font_bold_path = "C:\\Windows\\Fonts\\arial.ttf", "C:\\Windows\\Fonts\\arialbd.ttf"
    f_normal = 'ArialCustom' if os.path.exists(font_path) else 'Helvetica'
    f_bold = 'ArialCustomBold' if os.path.exists(font_bold_path) else 'Helvetica-Bold'
    
    if f_normal == 'ArialCustom':
        try: pdfmetrics.registerFont(TTFont('ArialCustom', font_path))
        except: f_normal = 'Helvetica'
    if f_bold == 'ArialCustomBold':
        try: pdfmetrics.registerFont(TTFont('ArialCustomBold', font_bold_path))
        except: f_bold = 'Helvetica-Bold'
        
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle('TitleStyle', parent=styles['Normal'], fontName=f_bold, fontSize=15, leading=18, alignment=1, textColor=colors.HexColor('#1E3A8A'))
...
# --- CHỨC NĂNG 6: QUẢN LÝ & THỐNG KÊ HỌC PHÍ (PDF) ---
    ...
    query_status = f'''
        ...
        GROUP BY h.id, h.ho_ten, h.hoc_phi_buoi, t.trang_thai
    '''
...
# --- CHỨC NĂNG 7: SỬA & XÓA DỮ LIỆU ---
    ...
    with st.form("add_student"):
        ...
        if st.form_submit_button("Thêm mới"):
            if ten:
                with engine.begin() as conn:
                    conn.execute(text("INSERT INTO hoc_sinh (ho_ten, lop_hoc, mon_hoc, hoc_phi_buoi) VALUES (:ten, :lop, :mon, :hp)"),
